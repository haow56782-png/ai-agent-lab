...
import { findGame, validatePrediction, type PredictionInput, type PredictionResult } from "./game.js";
...
export async function predict(input: PredictionInput): Promise<PredictionResult> {
  ...
  if (errors.length > 0) throw new Error(`Invalid prediction: ${errors.join("; ")}`);

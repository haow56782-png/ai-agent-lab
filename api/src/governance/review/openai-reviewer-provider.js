...
export class OpenAIReviewerProvider {
    ...
    async review(input) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                return createFallbackResult(`Provider rejected (HTTP ${response.status})`, model, "PROVIDER_REJECTED", summary);
            }
            // ── Parse response body ──────────────────────────────
            ...
            return createFallbackResult("Provider returned non-JSON response", model, "INVALID_JSON", text.slice(0, 300));
        }
        catch (err) {
            clearTimeout(timeoutId);
    ...
    async fetchWithRetry(url, apiKey, model, prompt, signal) {
        let lastError;
        let lastResponse;
    ...
    parseJSONOutput(text, model) {
        ...
        try {
            ...
            if (typeof parsed.verdict !== "string")
                return null;
            if (!["APPROVE", "APPROVE_WITH_CHANGES", "REJECT"].includes(parsed.verdict))
                return null;
            return {
                verdict: parsed.verdict,
                topRisks: Array.isArray(parsed.topRisks) ? parsed.topRisks.map((r) => ({
                    id: String(r.id ?? ""),
...
//# sourceMappingURL=openai-reviewer-provider.js.map
...
import { buildFixProgressMeta } from "./jobs/fix-progress-model.js";
function requireJob(job) {
    if (!job)
        throw createError(404, ERROR_CODES.NOT_FOUND, "Job not found");
    return job;
}
...
function getFixTypesFromResult(result, completedSteps) {
    if (Array.isArray(result.fixTypes))
        return result.fixTypes;
    if (Array.isArray(result.selectedFixes))
        return result.selectedFixes;
...
export async function getPublicFixStatus(jobId) {
    const job = requireJob(await jobRepo.getJob(jobId));
    if (job.job_type !== "fix") {
...
export async function getPublicDiff(jobId) {
    ...
    const headings = result.rawHeadings?.map((h) => h.text) || [];
...
function normalizeFindingDiffResult(raw) {
    const rawDiffs = Array.isArray(raw?.diffs) ? raw.diffs : [];
    const diffs = rawDiffs.map((diff, index) => {
    ...
    const findingDiffs = Array.isArray(raw?.findingDiffs) && raw.findingDiffs.length > 0
        ? raw.findingDiffs
            .map((diff) => normalizedById.get(diff.finding_id || diff.related_finding_ids?.[0]))
            .filter((diff) => !!diff)
...
export async function writeJobDownload(jobId, type, res) {
    ...
    if (type === "original") {
        ...
        if (!doc)
            throw createError(404, ERROR_CODES.NOT_FOUND, "Original document not found");

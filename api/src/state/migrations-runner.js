...
export async function getCurrentVersion(db) {
    try {
        ...
        await db.exec("CREATE TABLE IF NOT EXISTS _meta (key TEXT PRIMARY KEY, value TEXT NOT NULL);");
        await db.exec("INSERT OR IGNORE INTO _meta (key, value) VALUES ('schema_version', '0');");
        const row = await db.get("SELECT value FROM _meta WHERE key = 'schema_version'");

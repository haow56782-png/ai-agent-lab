...
export function detectReferenceMissingDoi(ctx) {
    const referenceEntries = (ctx.structureItems || []).filter((item) => item?.type === "reference_entry");
    if (referenceEntries.length === 0)
        return [];
    const missing = referenceEntries.find((entry) => !DOI_PATTERN.test(String(entry?.text || "")));
    if (!missing)
        return [];
...
export const referenceMissingDoiDetector = {
    ruleId: REFERENCE_MISSING_DOI_RULE_ID,
    label: REFERENCE_MISSING_DOI_LABEL,
    group: "参考文献",
    severity: "P2",

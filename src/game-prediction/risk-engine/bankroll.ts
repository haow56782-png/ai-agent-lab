...
const EXPOSURE_LIMITS: Record<RiskPreference, { maxExposure: number; warning: number; severe: number }> = {
...
const CONSECUTIVE_LOSS_LIMITS: Record<RiskPreference, number> = {
  conservative: 3,
  balanced: 5,
  aggressive: 7,
};

const STOP_LOSS_FACTORS: Record<RiskPreference, number> = {
  conservative: 0.15,
  balanced: 0.30,
  aggressive: 0.50,
};

const TAKE_PROFIT_FACTORS: Record<RiskPreference, number> = {
  conservative: 0.30,
  balanced: 0.60,
  aggressive: 1.00,
};
...
export function checkBankroll(
  ...
  let overbetLevel: BankrollStatus["overbetLevel"] = "none";
  if (exposureRatio > limits.severe * 2) overbetLevel = "critical";
  else if (exposureRatio > limits.severe) overbetLevel = "severe";
  else if (exposureRatio > limits.warning) overbetLevel = "warning";
...
export function calculateRiskOfRuin(
  ...
  if (bankroll <= 0 || betSize <= 0) return 1;
  if (betSize >= bankroll) return 1;
 ...
 */
export function checkOverbet(
  betSize: number,
  bankroll: number,
): { isOverbet: boolean; level: "none" | "warning" | "severe" | "critical"; message?: string } {
  if (bankroll <= 0) return { isOverbet: true, level: "critical", message: "Bankroll is empty." };
 ...
 */
export function calculateStopLoss(bankroll: number, riskPreference: RiskPreference): number {
 ...
 */
export function calculateTakeProfit(bankroll: number, riskPreference: RiskPreference): number {
 ...
 */
export function getRecommendedMaxBet(bankroll: number, riskPreference: RiskPreference): number {

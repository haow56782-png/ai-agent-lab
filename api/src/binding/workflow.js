...
// ─── Event Helpers ─────────────────────────────────
let _eventStartMs = 0;
function emitEvent(events, stage, status, data, error, startMs) {
 ...
 */
export async function runBindingWorkflow(options, store) {
    const events = [];
    ...
    async function transition(from, to, label, fn, spanName = `binding.${to.toLowerCase()}`) {
        state = from;
        ...
        try {
            ...
            return { ok: false, failure: to, error };
        }
    }
...
// ─── Finalize ─────────────────────────────────────
async function finalize(success, session, events, store) {

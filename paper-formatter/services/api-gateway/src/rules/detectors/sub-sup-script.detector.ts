 ...
 */
function buildRunSequence(paragraph: any): RunInfo[] {
  const runs: RunInfo[] = [];
...
function createSubSupDetection(input: {
  ...
  runIndex: number;
  isMissingSubscript?: boolean;
  isMissingSuperscript?: boolean;
}): RuleDetection {
  return {
    ruleId: input.ruleId,
    label: input.label,
 ...
 */
function detectMissingChemicalSubscripts(
  paragraphs: any[],
  ...
  for (const paragraph of paragraphs) {
    if (headingIndices.has(paragraph.index ?? paragraph.paragraphIndex)) continue;
 ...
 */
function detectMissingUnitSuperscripts(
  paragraphs: any[],
  headingIndices: Set<number>,
): RuleDetection[] {
  const detections: RuleDetection[] = [];
  ...
  for (const paragraph of paragraphs) {
    ...
    for (let ri = 0; ri < runs.length; ri++) {
      const run = runs[ri];
      if (run.superscript) continue; // Already superscript, skip
 ...
 */
function detectCitationNotSuperscript(
  paragraphs: any[],
  headingIndices: Set<number>,
  referenceIndices: Set<number>,
): RuleDetection[] {
  const detections: RuleDetection[] = [];

  for (const paragraph of paragraphs) {
    const pIdx = paragraph.index ?? paragraph.paragraphIndex ?? 0;
    if (headingIndices.has(pIdx)) continue;
    if (referenceIndices.has(pIdx)) continue; // Skip reference section itself
 ...
 */
export function detectSubSupScript(ctx: ParsedDocumentContext): RuleDetection[] {
  const paragraphs = ctx.paragraphs || [];
  if (paragraphs.length === 0) return [];

  const headingIndices = new Set<number>(
    (ctx.headings || [])
      .map((h: any) => h.index ?? h.paragraphIndex)
      .filter((v: unknown) => typeof v === "number"),
  );

  const referenceIndices = new Set<number>(
    (ctx.structureItems || [])
      .filter((s: any) => s.type === "reference_entry" || s.type === "references_header")
      .map((s: any) => s.index)
      .filter(Boolean),
  );
...
export const subSupScriptDetector: FormatRuleDetector = {
  ruleId: SUB_SUP_SCRIPT_RULE_ID,

...
import type { ConstitutionalImpact } from "./governance-memory-types.js";
import { computeDecisionHash } from "./governance-memory-utils.js";

/* ── In-Memory ADR Store ────────────────────────────────── */

export class InMemoryAdrStore {
  private entries: Map<string, AdrStoreEntry> = new Map();
  private nextId: number = 1;
  private links: Map<string, Set<string>> = new Map(); // source → targets
  private reverseLinks: Map<string, Set<string>> = new Map(); // target → sources
  ...
  appendADR(adr: CognitiveAdrJson): void {
    ...
    if (existing) {
      throw new Error(
        `ADR ${adr.id} already exists — overwrite is not permitted (append-only)`,
      );
    }

    const entry: AdrStoreEntry = {
      adr: { ...adr },
  ...
  linkRelatedADR(
    sourceId: string,
    targetId: string,
    relationship: "SUPERSEDES" | "RELATED",
  ): void {
  ...
  validateADR(input: CognitiveAdrInput): AdrValidationResult {
    ...
    if (!input.decision?.trim()) errors.push("decision is required");
    if (!input.context?.trim()) errors.push("context is required");
    if (!input.problemStatement?.trim()) errors.push("problemStatement is required");
    ...
    const validImpacts: ConstitutionalImpact[] = [
      "NONE",
      "BOUNDARY",
      "INVARIANT",
      "AUTHORITY",
    ];
    if (!validImpacts.includes(input.constitutionalImpact as ConstitutionalImpact)) {
      errors.push(
        `constitutionalImpact must be one of: ${validImpacts.join(", ")}`,
      );
    }

    // Reversal conditions required for constitutional ADRs
    if (
      input.constitutionalImpact !== "NONE" &&
      (!input.reversalConditions || input.reversalConditions.length === 0)
    ) {
      errors.push(
        "reversalConditions required for constitutional-impact ADRs " +
          `(constitutionalImpact=${input.constitutionalImpact})`,
      );
    }
  ...
  getRelatedADRs(adrId: string): CognitiveAdrJson[] {
    ...
    for (const id of linked) {
      const entry = this.entries.get(id);
      if (entry) result.push({ ...entry.adr });
    }
    return result;
  }
  ...
  private computeHash(input: CognitiveAdrInput): string {
    const touched = (input.relatedInvariants ?? []).slice().sort();
    return computeDecisionHash(
      `ADR-${String(this.nextId).padStart(4, "0")}`,
      input.decision,
      input.context,
      input.freezeVersion ?? "",
      touched.join(","),
  ...
  private registerLink(sourceId: string, targetId: string): void {

...
interface FloatingOverlapCandidate {
  page: number;
  paragraphIndex: number;
  snippet: string;
  contextBefore?: string;
...
function estimatePageFromParagraphIndex(paragraphIndex: number): number {
  return Math.max(1, Math.floor(paragraphIndex / 8) + 1);
}

function shouldSkipFloatingObject(image: ParserImageMeta): boolean {
  if (image.object_type !== "floating") return true;
  if (image.behind_text) return true;
  if (!image.overlap_risk && !image.allow_overlap) return true;
  if ((image.width_pt || 0) < 18 && (image.height_pt || 0) < 18) return true;
  return false;
}

function resolveNearbyParagraph(paragraphs: any[], image: ParserImageMeta): NearbyParagraph | null {
...
function computeTextMetrics(ctx: ParsedDocumentContext, paragraph: any, snippet: string): TextMetrics {
  ...
  const fontSizePt = Math.max(10, paragraph?.runs?.find((run: any) => run?.size_pt)?.size_pt || 12);
...
function computeOverlapRatio(image: ParserImageMeta, metrics: TextMetrics): number {
  ...
  const minArea = Math.max(
    1,
    Math.min(
      Math.max(widthPt * Math.max(heightPt, metrics.lineHeightPt), 1),
      Math.max(metrics.textWidthPt * metrics.textHeightPt, 1),
    ),
  );
  return overlapArea / minArea;
}

function buildCandidate(ctx: ParsedDocumentContext, image: ParserImageMeta, nearby: NearbyParagraph): FloatingOverlapCandidate | null {
...
export function detectFloatingObjectOverlap(ctx: ParsedDocumentContext): RuleDetection[] {
  const images = (ctx.images || []) as ParserImageMeta[];
  return images
    ...
    .filter((candidate): candidate is FloatingOverlapCandidate => candidate !== null)
    ...
    .map((candidate) => {
      ...
      } satisfies RuleDetection;
    });
}

export const floatingObjectOverlapDetector: FormatRuleDetector = {
  ruleId: FLOATING_OBJECT_OVERLAP_RULE_ID,
  label: FLOATING_OBJECT_OVERLAP_LABEL,
  group: "图形对象 & 正文",
  severity: "P1",

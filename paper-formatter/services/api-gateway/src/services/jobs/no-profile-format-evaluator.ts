...
export function buildAutoFormatIssues(sections: any[], paragraphs: any[]): AutoFormatIssue[] {
  const formatIssues: AutoFormatIssue[] = [];
  const detectedFonts = new Set<string>();
  const detectedFontSizes = new Set<number>();
  ...
  for (const paragraph of paragraphs) {
    ...
    if (paragraph.runs && paragraph.runs.length > 0) {
      ...
      for (const run of paragraph.runs) {
        if (run.name) detectedFonts.add(run.name);
        if (run.size_pt) detectedFontSizes.add(Math.round(run.size_pt));
      }
  ...
  const headingParagraphs = paragraphs.filter((paragraph: any) => paragraph.is_heading || paragraph.style?.startsWith("Heading"));
  if (headingParagraphs.length > 0) {
    const headingFonts = [...new Set(headingParagraphs.flatMap((heading: any) => (heading.runs || []).map((run: any) => run.name).filter(Boolean)))];

...
function estimateTableRisk(table) {
    const rows = Number(table?.rows || 0);
    const cols = Number(table?.cols || 0);
    const maxCellLength = Math.max(0, ...((table?.data || []).flatMap((row) => (row?.cells || []).map((cell) => String(cell || "").trim().length))));
    return rows >= 8 || cols >= 6 || maxCellLength >= 48;
}
export function detectTableKeepTogether(ctx) {
    const tables = ctx.tables || [];
    const riskyTable = tables.find((table) => estimateTableRisk(table));
    if (!riskyTable)
        return [];
...
export const tableKeepTogetherDetector = {
    ruleId: TABLE_KEEP_TOGETHER_RULE_ID,
    label: TABLE_KEEP_TOGETHER_LABEL,
    group: "图表 & 题注",
    severity: "P2",

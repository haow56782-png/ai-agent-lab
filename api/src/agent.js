...
export function createAgent(config = {}) {
    ...
    const maxIterations = config.maxIterations ?? getConfig("agent.maxIterations");
    const llm = config.llm ?? createLLM();
    const enableBoundaryRouting = config.enableBoundaryRouting ?? false;
    const opusModel = config.opusModel ?? getConfig("llm.opusModel");
    const ctx = {
        projectRoot,
        designSystemPath: projectRoot,
        toolTimeoutMs: getConfig("llm.timeoutMs"),
    };
    if (config.sessionId) {
        setSession(config.sessionId);
    }
    const defaultSystem = config.systemPrompt ??
        [
    ...
    async function* runStream(userInput) {
        const traceId = beginTrace();
        logger.setTraceId(traceId);

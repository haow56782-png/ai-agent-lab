...
function detectionToRuleHitItem(detection) {
    return {
...
function findDetectionItems(detections, ruleIds) {
...
function createDefaultFigureItems(detections) {
    ...
    return [
        ...
        ...(captionItems.length > 0 ? captionItems : [
            [FIGURE_CAPTION_POSITION_LABEL, "pass"],
            [TABLE_CAPTION_POSITION_LABEL, "pass"],
        ]),
        ...(tableItems.length === 0 ? [[TABLE_KEEP_TOGETHER_LABEL, "pass"]] : []),
    ];
}
            ...
            items: pageItems.length > 0
                ...
                : (() => {
                    if (sections.length > 0) {
                        const s = sections[0];
                        return [
                            [`${PAGE_MARGIN_LABEL} ${s.margin_top_mm}/${s.margin_bottom_mm}/${s.margin_left_mm}/${s.margin_right_mm} mm`, "pass"],
                            [GUTTER_LABEL, "pass"],
                        ];
                    }
                    return [
                        [PAGE_MARGIN_LABEL, "pass"],
                        [GUTTER_LABEL, "pass"],
                    ];
                })(),
        },
        {
            ...
            items: [
                ...(styleItems.length > 0
                    ? styleItems
                    : [
                        [BODY_STYLE_LABEL, "pass"],
                        [HEADING_SPACING_LABEL, "pass"],
                        [HEADING_HIERARCHY_LABEL, "pass"],
                        [FOOTNOTE_STYLE_LABEL, "pass"],
                    ]),
            ],
        },
        {
            ...
            items: [
                ...(sectionItems.length > 0
                    ? sectionItems
                    : [
                        [FRONT_MATTER_ROMAN_LABEL, "pass"],
                        [PAGE_SECTION_LABEL, "pass"],
                        [FOOTER_ALIGNMENT_LABEL, "pass"],
                    ]),
            ],
        },
        { cat: "图表 & 题注", items: figureItems },
        {
            ...
            items: [
                ...(tocItems.length > 0
                    ? tocItems
                    : [
                        [TOC_REFRESH_LABEL, "pass"],
                        [FIGURE_DIRECTORY_LABEL, "pass"],
                    ]),
            ],
        },
        {
            ...
            items: [
                ...
                ["悬挂缩进", "pass"],
                ...(referenceItems.length > 0 ? referenceItems : [[REFERENCE_MISSING_DOI_LABEL, "pass"]]),
            ],
        },
    ];
}

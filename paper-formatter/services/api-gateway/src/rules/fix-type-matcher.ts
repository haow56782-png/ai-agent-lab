...
const FINDING_MATCHERS: Partial<Record<FixType, RegExp>> = {
...
const FORMATTER_MATCHERS: Partial<Record<FixType, RegExp>> = {
...
export function matchFindingForFixType(fixType: FixType, findings: FindingContract[], index: number): FindingContract | null {
...
export function matchFormatterFindingIdForFixType(formatterDiff: any, fixType: FixType, index: number): string | undefined {
  const diffs = Array.isArray(formatterDiff?.diffs) ? formatterDiff.diffs : [];
  if (diffs.length === 0) return undefined;
  const mapped = diffs.find((diff: any) => diff?.finding_id && ruleMatchesFixType(diff?.rule_id, fixType));
  if (mapped?.finding_id) return mapped.finding_id;
  const matcher = FORMATTER_MATCHERS[fixType];
  const matched = matcher
    ? diffs.find((diff: any) => diff?.finding_id && matcher.test(`${diff.element || ""} ${diff.position || ""} ${diff.note || ""}`))
    : null;

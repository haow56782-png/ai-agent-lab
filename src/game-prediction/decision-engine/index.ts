...
function validateInput(input: DecisionInput): void {
  ...
  if (input.bankroll == null || input.bankroll < 0) throw new Error("bankroll must be a non-negative number");
  if (input.betSize == null || input.betSize < 0) throw new Error("betSize must be a non-negative number");
  if (!input.riskPreference) throw new Error("riskPreference is required");
  ...
  if (!input.domainSignal) throw new Error("domainSignal is required");
}

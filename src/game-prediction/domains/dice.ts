...
export function getRiskProfile(): RiskProfile {
  return {
    ...
    description:
      "Dice is a low-volatility game with fixed 1/6 probability per outcome. "
...
export function parseDiceInput(input: DiceInput): ValidatedDiceInput {
...
export function calculateProbability(_parsed: ValidatedDiceInput, useHistory: boolean): number {
  if (useHistory) {
...
export function calculateConfidence(useHistory: boolean, historyLength: number): number {
  if (useHistory && historyLength > 0) {
    // Pattern analysis is less reliable on small samples
    if (historyLength < 10) return 0.60;
    if (historyLength < 50) return 0.75;
    return 0.85;
  }
...
export function getExplanation(
  ...
  if (useHistory && input.history.length > 0) {
    parts.push(
      `Historical analysis over ${input.history.length} past rolls shows no deviation from expected distribution.`,
    );
  }

  if (probability < 0.3) {
    parts.push(
      `Single-number bets carry negative expected value due to the ${(HOUSE_EDGE * 100).toFixed(1)}% house edge.`,
    );
  }
...
export function generateSignal(input: DiceInput): Signal {

 ...
 */
export async function runAgent(input: AnalyzeInput): Promise<AnalyzeResult> {
  ...
  let signal: SignalOutputContract | undefined;
  try {
    const jsonMatch = output.match(/\{[\s\S]*"signal_id"[\s\S]*"domain"[\s\S]*"tool_name"[\s\S]*\}/);
    if (jsonMatch) {
      const parsed = JSON.parse(jsonMatch[0]) as SignalOutputContract;

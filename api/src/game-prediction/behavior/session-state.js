 ...
 */
export function buildSessionState(betHistory, bankrollHistory, sessionDurationMinutes, recentResults) {
    ...
    let streakType = recentResults[recentResults.length - 1] ?? "win";
    let streakCount = 0;
    ...
    for (const b of betHistory) {
        ...
        totalProfitLoss += pl;
        if (b.bankrollAfter > peakBankroll)
            peakBankroll = b.bankrollAfter;
    }

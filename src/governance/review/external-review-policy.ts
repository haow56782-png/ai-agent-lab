 ...
 */
const SEMANTIC_BOUNDARY_MAP: Record<TriggerCategory, ExecutionBoundary> = {
  /* ── E3: Opus Arbitration + External Review + Freeze Gate ── */
...
export type TriggerCategory =
  ...
  /* ── L0 / L1: Typically no external review ──────────── */
  | "test_changes"
  | "internal_refactor"
  | "mock_provider"
...
const TRIGGER_LEVELS: Record<TriggerCategory, ReviewLevel> = {
...
const TRIGGER_DESCRIPTIONS: Record<TriggerCategory, string> = {
...
const REVIEWER_MODEL_POLICY: Record<ReviewLevel, ReviewerModelRequirement> = {
  [ReviewLevel.L0]: {
 ...
 */
export function evaluateTrigger(category: TriggerCategory): TriggerEvaluation {
 ...
 */
export function getReviewLevel(categories: TriggerCategory[]): ReviewLevel {
  let max: ReviewLevel = ReviewLevel.L0;
...
export function shouldRequireExternalReview(level: ReviewLevel): boolean {
  return level === ReviewLevel.L2 || level === ReviewLevel.L3;
}

export function shouldEscalateToFreezeGate(level: ReviewLevel): boolean {
  return level === ReviewLevel.L3;
}

export function getRequiredReviewerModel(level: ReviewLevel): ReviewerModelRequirement {
 ...
 */
export function classifyExecutionBoundary(categories: TriggerCategory[]): ExecutionBoundary {
  if (!categories || categories.length === 0) return ExecutionBoundary.E0;
 ...
 */
export function boundaryRequiresPlan(boundary: ExecutionBoundary): boolean {
 ...
 */
export function boundaryRequiresArbitration(boundary: ExecutionBoundary): boolean {
 ...
 */
export function getRecommendedExecutor(boundary: ExecutionBoundary): string {
  switch (boundary) {
 ...
 */
export function describeExecutionBoundary(boundary: ExecutionBoundary): string {
  switch (boundary) {
 ...
 */
export function evaluateRoutingDrift(entry: RoutingDecisionLog): {
...
function executionBoundaryToNumber(boundary: ExecutionBoundary): number {
  switch (boundary) {

 ...
 */
function patternSeverityScore(patterns) {
    return patterns.reduce((s, p) => s + (SEVERITY_WEIGHTS[p.severity] ?? 0), 0);
}
/**
 * Determine behavior state from all signals.
 */
export function determineBehaviorState(patterns, tiltEval, fatigue) {
    // Check for stop-required conditions first (highest priority)
    const hasCriticalLossChasing = patterns.some((p) => p.pattern === "loss_chasing" && p.severity === "critical");
    const hasCriticalMaxLoss = patterns.some((p) => p.pattern === "over_max_loss" && p.severity === "critical");
    const hasCriticalMismatch = patterns.some((p) => p.pattern === "risk_mismatch" && p.severity === "critical");
    if (hasCriticalLossChasing || hasCriticalMaxLoss)
        return "STOP_REQUIRED";
    if (tiltEval.isTilt && tiltEval.tiltScore > 0.7)
        return "STOP_REQUIRED";
    // Check for tilt / chasing loss
    if (tiltEval.isTilt)
        return "TILT";
    if (patterns.some((p) => p.pattern === "loss_chasing" && p.severity === "high"))
        return "CHASING_LOSS";
    // Check for fatigue
    if (fatigue.isFatigued && fatigue.fatigueLevel === "severe")
        return "STOP_REQUIRED";
    if (fatigue.isFatigued && fatigue.fatigueLevel === "moderate")
        return "FATIGUE";
    // Check for caution conditions
    if (hasCriticalMismatch)
        return "CAUTION";
    if (patternSeverityScore(patterns) >= 4)
        return "CAUTION";
    // Check misc patterns
 ...
 */
export function determineIntervention(behaviorState, patterns) {
 ...
 */
export function computeDecisionAdjustment(behaviorState, currentAction) {
    switch (behaviorState) {
        case "STOP_REQUIRED":
        case "TILT":
            return {
 ...
 */
export function generateInterventionReasoning(behaviorState, interventionLevel, patterns, tiltEval, fatigue) {
    const parts = [`Behavior state: ${behaviorState}`];
    if (patterns.length > 0) {
        const topPattern = patterns.reduce((a, b) => SEVERITY_WEIGHTS[a.severity] > SEVERITY_WEIGHTS[b.severity] ? a : b);
 ...
 */
export function collectBehaviorWarnings(patterns, tiltEval, fatigue) {
    const warnings = [];

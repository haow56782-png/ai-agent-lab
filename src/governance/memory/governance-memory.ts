...
export class GovernanceMemoryStore {
  private entries: GovernanceMemoryEntry[] = [];
  private freezeSnapshots: Map<string, FreezeSnapshot> = new Map();
   ...
   */
  createGovernanceMemoryEntry(
    input: GovernanceMemoryEntryInput,
    ...
    const decisionHash = computeDecisionHash(
      ...
      input.reviewLevel,
      input.freezeVersion,
      input.invariantsTouched.slice().sort(),
    );
  ...
  appendGovernanceMemoryEntry(entry: GovernanceMemoryEntry): void {
    ...
    this.nextEntryId = Math.max(
      this.nextEntryId,
      parseInt(entry.id.split("-")[1], 10) + 1,
    );
  }
  ...
  thawFreezeSnapshot(
    ...
    if (snap.status !== "FROZEN") {
      throw new Error(
        `Freeze snapshot ${snapshotId} is not FROZEN (status=${snap.status})`,
      );
    }
  ...
  validateGovernanceProposal(
    ...
    const decisionHash = computeDecisionHash(
      ...
      reviewLevel,
      "",
      invariantsTouched.slice().sort(),
    );

    const checks: GovernanceProposalCheck["checks"] = [];
    ...
    const hasExternalReviewer = reviewerModels.some(
      (m) => m !== "claude-opus" && m !== "none",
    );
    checks.push({
      rule: "External reviewer cannot finalize freeze",
      passed: !(hasExternalReviewer && reviewLevel === "L3") || arbitrationOwner === "claude-opus",
      detail:
        !(hasExternalReviewer && reviewLevel === "L3") || arbitrationOwner === "claude-opus"
    ...
    const hasMiniFinalReviewer =
      reviewLevel === "L3" &&
      reviewerModels.some(
        (m) =>
          m.includes("mini") || m.includes("flash"),
      );
    checks.push({
    ...
    const touchesConstitutional = invariantsTouched.some((inv) =>
      ["INV-001", "INV-002", "INV-004", "INV-005"].includes(inv),
    );
    checks.push({
      rule: "Constitutional changes require L3 Freeze Gate",
      passed: !touchesConstitutional || reviewLevel === "L3",
      detail:
        !touchesConstitutional || reviewLevel === "L3"

...
export function getRiskProfile(): RiskProfile {
  return {
    ...
    description:
      "Crash is a high-volatility game where you bet on how high the multiplier "
...
export function parseCrashInput(input: CrashInput): ValidatedCrashInput {
  const targetMultiplier = input.targetMultiplier;
  if (typeof targetMultiplier !== "number" || targetMultiplier < 1.0) {
    throw new Error(
      `Invalid targetMultiplier: ${targetMultiplier}. Must be >= 1.0.`,
    );
  }
 ...
 */
export function calculateProbability(multiplier: number): number {
...
export function calculateConfidence(
  ...
  if (probability > 0.5) return 0.80;
  if (probability > 0.2) return 0.75;
  // Very low probabilities have higher variance in practice
  if (historyLength > 100) return 0.70;
  return 0.65;
}
...
export function getExplanation(
  ...
  if (parsed.targetMultiplier <= 1.5) {
    parts.push(
      "Low multipliers crash less often, but the payout barely covers the risk.",
    );
  } else if (parsed.targetMultiplier <= 3.0) {
    parts.push(
      "Medium multipliers offer better payouts but crash more frequently than they appear to.",
    );
  } else {
    parts.push(
      "High multipliers are statistically unlikely to hit. The house edge compounds at higher targets.",
    );
  }

  if (parsed.previousCrashPoints.length > 0) {
    parts.push(
      `Based on ${parsed.previousCrashPoints.length} observed crash points. Past crashes do not predict future ones.`,
    );
  }
...
export function generateSignal(input: CrashInput): Signal {

...
function nextState(current, transition) {
    const flow = [
...
export function createFreezeEntry(adrId, actor) {
    const event = {
        from: "DRAFT",
        to: "DRAFT",
...
export function transitionFreeze(entry, transition, actor, reason, evidence) {
    const allowed = VALID_TRANSITIONS[entry.state];
    if (!allowed.includes(transition)) {
        throw new Error(`Invalid transition "${transition}" from state "${entry.state}". ` +
...
export function getFreezeStatus(entry) {
...
//# sourceMappingURL=freeze-state-machine.js.map
...
export function parseFindingDocumentQuery(query: unknown): FindingDocumentQuery {
  ...
  return {
    ...
    status: readString(query, "status") as FindingDocumentQuery["status"],
    severity: readString(query, "severity") as FindingDocumentQuery["severity"],
    ruleId: readString(query, "rule_id"),
    ruleGroup: readString(query, "rule_group"),
  };
}

export function toFindingListQuery(query: FindingDocumentQuery): FindingListQuery {
  return {
...
export function parseFindingSyncCommand(body: unknown): FindingSyncCommand {
  const canonicalDocumentId = readString(body, "document_id");
  const findings = !body || typeof body !== "object" ? undefined : (body as Record<string, unknown>).findings;
  ...
  return {
    ...
    canonicalDocumentId,
    findings: findings as FindingSyncCommand["findings"],
  };
}

export function parseP1ExemptionCommand(body: unknown): P1ExemptionCommand {
  const canonicalDocumentId = readString(body, "document_id");
  const exemptedFindingIds = readStringArray(body, "exempted_finding_ids");
  const actorId = readString(body, "actor_id");
  const actorRole = readString(body, "actor_role") as P1ExemptionCommand["actorRole"] | undefined;
  const reason = readString(body, "reason");
  const acknowledged = Boolean(body && typeof body === "object" && (body as Record<string, unknown>).acknowledged);

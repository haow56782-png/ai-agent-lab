 ...
 */
export function detectLossChasing(betHistory, recentResults) {
    if (recentResults.length < LOSS_CHASING_MIN_LOSSES)
        return null;
    // Count consecutive losses from most recent
    let consecutiveLosses = 0;
    for (let i = recentResults.length - 1; i >= 0; i--) {
        if (recentResults[i] === "loss")
            consecutiveLosses++;
        else
            break;
    }
    if (consecutiveLosses < LOSS_CHASING_MIN_LOSSES)
        return null;
 ...
 */
export function detectOverMaxLoss(betHistory, maxLoss) {
 ...
 */
export function detectHighFrequency(lastBetTimestamps, sessionDurationMinutes, totalBets) {
    if (lastBetTimestamps.length < 3)
        return null;
 ...
 */
export function detectOverbetAfterWin(betHistory) {
 ...
 */
export function detectRiskMismatch(betHistory, riskPreference, sessionState) {
    if (betHistory.length < 2)
        return null;
    // Define max appropriate bet sizes per risk preference (as % of bankroll)
    const maxBetFraction = {
        conservative: 0.05,
        balanced: 0.10,
        aggressive: 0.20,
    };
 ...
 */
export function detectAllPatterns(betHistory, recentResults, maxLoss, riskPreference, sessionState) {
    const patterns = [];
    const lossChasing = detectLossChasing(betHistory, recentResults);
    if (lossChasing)
        patterns.push(lossChasing);
    const overMaxLoss = detectOverMaxLoss(betHistory, maxLoss);
    if (overMaxLoss)
        patterns.push(overMaxLoss);

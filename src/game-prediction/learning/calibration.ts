 ...
 */
export function computeBrierScore(records: LearningRecord[]): number {
 ...
 */
export function computeRollingAccuracy(records: LearningRecord[], window = 50): number {
  if (records.length === 0) return 0;
 ...
 */
export function computeCalibrationBuckets(records: LearningRecord[]): CalibrationBucket[] {
  if (records.length === 0) return [];

  const buckets: Map<string, { predictedSum: number; actualSum: number; count: number }> = new Map();
 ...
 */
export function computeConfidenceDrift(records: LearningRecord[]): number {
 ...
 */
export function computeStrategyReliability(records: LearningRecord[]): Record<string, number> {
 ...
 */
export function computeCalibration(records: LearningRecord[]): CalibrationMetrics {
  return {

...
export const DETECTOR_TO_CANONICAL_RULE_ID = {
...
export const CANONICAL_RULE_ID_EXEMPTIONS = {};
...
function getProfileRuleIds(profile) {
    return new Set([
        ...((profile?.rules_json || []).map((rule) => String(rule?.ruleId || rule?.rule_id || "")).filter(Boolean)),
        ...((profile?.style_map || []).map((rule) => String(rule?.ruleId || rule?.rule_id || "")).filter(Boolean)),
    ]);
}
...
export function detectionMatchesRuleId(detection, ruleId) {

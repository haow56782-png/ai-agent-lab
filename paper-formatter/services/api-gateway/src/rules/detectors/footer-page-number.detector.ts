...
function resolveFooterNumberFormat(section: any): string {
  ...
  if (ROMAN_PATTERN.test(footerText)) return "roman";
  if (ARABIC_PATTERN.test(footerText)) return "arabic";
...
export function detectFooterPageNumber(ctx: ParsedDocumentContext): RuleDetection[] {
  ...
  const misalignedFooter = sections.find((section: any) => {
    const footerText = String(section?.footer_text || "").trim();
...
export const footerPageNumberDetector: FormatRuleDetector = {
  ruleId: FOOTER_ALIGNMENT_RULE_ID,
  label: FOOTER_ALIGNMENT_LABEL,
  group: "分节 & 页码",
  severity: "P2",

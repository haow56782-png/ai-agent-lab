...
export function detectTocRefreshReview(ctx) {
    const headings = ctx.headings || [];
    const tocEntry = (ctx.structureItems || []).find((item) => item?.type === "toc");
    if (headings.length < 3 || tocEntry)
        return [];
...
export const tocRefreshDetector = {
    ruleId: TOC_REFRESH_RULE_ID,
    label: TOC_REFRESH_LABEL,
    group: "目录 & 域",
    severity: "P2",

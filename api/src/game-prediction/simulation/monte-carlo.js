...
export function runMonteCarlo(params) {
    ...
    for (let sim = 0; sim < params.simulations; sim++) {
        ...
        for (let r = 0; r < params.rounds; r++) {
            ...
            if (params.stopLoss != null && loss >= params.stopLoss)
                break;
            if (params.takeProfit != null && profit >= params.takeProfit)
                break;
        }
...
/* ─── Aggregate results ─── */
export function aggregateResults(raw, input, payoutMultiplier) {
...
function buildRecommendation(ev, ruinProb, maxDD, bankroll) {
    if (ruinProb > 0.3) {
...
function buildWarnings(ev, ruinProb, maxDD, bankroll, winRate) {
    const warnings = [];
    if (ev < 0)
        warnings.push(`Negative expected value (${(ev * 100).toFixed(1)}% per bet). Long-term losses expected.`);
    if (ruinProb > 0.1)
        warnings.push(`High ruin probability (${(ruinProb * 100).toFixed(0)}%). Consider reducing bet size.`);
    if (maxDD > bankroll * 0.4)
        warnings.push(`Large maximum drawdown (${((maxDD / bankroll) * 100).toFixed(0)}% of bankroll).`);
    if (winRate < 0.2)
        warnings.push(`Low win rate (${(winRate * 100).toFixed(0)}%). Extended losing streaks are likely.`);
    if (ruinProb > 0.3)
        warnings.push("Risk of ruin exceeds 30%. Strategy is not sustainable.");
    return warnings;
}

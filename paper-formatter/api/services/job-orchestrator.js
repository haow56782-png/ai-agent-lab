...
export async function startAnalyzeJob(input) {
    if (!input.legacyDocId)
        throw createError(400, ERROR_CODES.VALIDATION_ERROR, "docId is required");
    const doc = await docRepo.getDocument(input.legacyDocId);
    if (!doc)
        throw createError(404, ERROR_CODES.NOT_FOUND, `Document ${input.legacyDocId} not found`);
...
export async function startFormatJob(input) {
...
export async function startFixJob(input) {
    if (!input.profileId)
        throw createError(400, ERROR_CODES.VALIDATION_ERROR, "profileId is required");
    const doc = await resolveDocument(input.legacyDocId, input.jobId);
    const requestedFixTypes = Array.isArray(input.selectedFixes) && input.selectedFixes.length > 0
        ...
        : Array.isArray(input.fixTypes) && input.fixTypes.length > 0
            ? input.fixTypes
            : [...SUPPORTED_FIX_TYPES];
    const invalidFixTypes = requestedFixTypes.filter((type) => !SUPPORTED_FIX_TYPES.includes(type));
    ...
    void processFixJob(jobId, doc, input.profileId, requestedFixTypes, input.jobId);

export const REQUIRED_THESIS_SUBSETS = [
    "page_canvas",
    "cover",
...
export const CANONICAL_RULE_GROUPS = [
    {
        cat: "01. 页面与纸张",
...
function inferRuleType(group, text) {
    if (["citation", "reference", "keywords", "translated_source"].includes(group.thesisSubset))
        return "content";
    if (["page_canvas", "header_footer", "page_number", "section_break", "directory_field"].includes(group.thesisSubset))
        return "layout";
    if (["cover", "originality_statement", "authorization", "abstract_zh", "abstract_en", "toc", "heading", "appendix", "acknowledgement"].includes(group.thesisSubset))
        return "structure";
    if (/引用|参考文献|关键词|DOI|URL/.test(text))
        return "content";
    if (/独立成页|另页|分节符|页码|页眉|页脚|目录域|TOC/.test(text))
        return "layout";
    if (/标题|摘要|目录|章节|附录|致谢|封面|声明|授权书/.test(text))
        return "structure";
    return "format";
}
export function buildCanonicalRuleCatalog() {

...
export function createLLM(config) {
    ...
    const defaultModel = config?.model ?? process.env.LLM_MODEL ?? getConfig("llm.model");
    const timeoutMs = config?.timeoutMs ?? getConfig("llm.timeoutMs");
    const retryMax = config?.retryMax ?? getConfig("llm.retry.max");
    const retryDelayMs = config?.retryDelayMs ?? getConfig("llm.retry.delayMs");
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                const response = await client.chat.completions.create({
                    model: overrides?.model ?? defaultModel,
                    messages,
                    temperature: overrides?.temperature ?? config?.temperature ?? getConfig("llm.temperature"),
                    max_tokens: overrides?.maxTokens ?? config?.maxTokens ?? getConfig("llm.maxTokens"),
                });
    ...
    async function* stream(messages, overrides) {

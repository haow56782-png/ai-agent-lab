...
import { getPublicDiff, getPublicFixStatus, getPublicJob, writeJobDownload, } from "../services/job-queries.js";
import { startAnalyzeJob, startFixJob, startFormatJob, } from "../services/job-orchestrator.js";
import { parseAnalyzeJobCommand, parseFixJobCommand, parseFormatJobCommand, } from "../dto/job-document-requests.js";
...
jobRoutes.get("/:jobId/download", async (req, res, next) => {
    try {
        await writeJobDownload(req.params.jobId, req.query.type, res);
    }
    catch (err) {
        next(err);
    }

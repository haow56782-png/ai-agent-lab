...
export async function resolveDocument(legacyDocId, sourceJobId) {
    ...
    if (!resolvedLegacyDocId && requestedJobId) {
        ...
        if (!sourceJob)
            throw createError(404, ERROR_CODES.NOT_FOUND, `Job ${requestedJobId} not found`);
    ...
    if (!documentRecord)
        throw createError(404, ERROR_CODES.NOT_FOUND, `Document ${resolvedLegacyDocId} not found`);
    return documentRecord;
}
export async function loadUploadedDocumentBuffer(documentRecord) {

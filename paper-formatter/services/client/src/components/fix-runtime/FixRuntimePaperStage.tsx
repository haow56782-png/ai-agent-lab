...
export const FixRuntimePaperStage: React.FC<Props> = ({
    ...
    chars.forEach((char, charIndex) => {
      ...
      nodes.push(
        ...
        >
          {content}
          {replacement && (
            <sup className={replacement.active ? 'fix-paper-replace-note is-live' : 'fix-paper-replace-note'}>
              {replacement.payload}
            </sup>
          )}
        </span>,
          ...
          {page.blocks.map((block, index) => {
            ...
            if (block.type === 'h1') {
              return live && currentParagraphIndex !== null
                ...
                : <h1 key={`${block.type}-${index}`} className="fix-paper-title" style={getBlockStyle(block)}>{block.content}</h1>;
            }
            if (block.type === 'h2') {
              return live && currentParagraphIndex !== null
                ? renderRichText(block, index, currentParagraphIndex, 'fix-paper-chapter', 'h2')
                : <h2 key={`${block.type}-${index}`} className="fix-paper-chapter" style={getBlockStyle(block)}>{block.content}</h2>;
            }
            if (block.type === 'h3') {
              return live && currentParagraphIndex !== null
                ? renderRichText(block, index, currentParagraphIndex, 'fix-paper-subchapter', 'h3')
                : <h3 key={`${block.type}-${index}`} className="fix-paper-subchapter" style={getBlockStyle(block)}>{block.content}</h3>;
            }
            ...
            if (block.type === 'reference') {
              return live && currentParagraphIndex !== null
                ...
                : <p key={`${block.type}-${index}`} className="fix-paper-reference" style={getBlockStyle(block)}>{block.content}</p>;
            }
            return live && currentParagraphIndex !== null
              ? renderRichText(block, index, currentParagraphIndex, 'fix-paper-paragraph', 'p')
              : <p key={`${block.type}-${index}`} className="fix-paper-paragraph" style={getBlockStyle(block)}>{block.content}</p>;
          })}
        </div>
  ...
  return (
    <div className="fix-runtime-paperframe">

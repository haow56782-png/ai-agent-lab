...
export function needsRefinement(review) {
    const lower = review.toLowerCase();
    const hasSignal = lower.includes("issue") ||
        lower.includes("fix") ||
...
async function safeChat(llm, messages, fallback) {
    try {
 ...
 */
export async function runWorkflow(task, context) {
    const llm = createLLM({ temperature: 0.6 });
    // --- Plan ---
    const plan = await safeChat(llm, [
        {
            role: "system",
    ...
    // --- Execute ---
    const output = await safeChat(llm, [
        {
            role: "system",
    ...
    // --- Review ---
    const review = await safeChat(llm, [
        {
            role: "system",

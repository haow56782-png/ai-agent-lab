...
import type { GovernanceMemoryEntry } from "../memory/governance-memory.js";
import { ReviewLevel } from "./external-review-policy.js";
import { InvariantRegistry } from "../memory/invariant-registry.js";
import { GovernanceCostGuard, type CostGuardConfig, type CostGuardTier, type ReplayScope } from "../memory/governance-cost-guard.js";
import {
  ...
  checkReplayBlock,
} from "../memory/governance-memory-file.js";
import {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--cost-guard":
        ...
        break;
      case "--max-cost-tier":
        opts.maxCostTier = args[++i] as CostGuardTier;
        break;
      ...
      case "--disable-review-cache":
        ...
        break;
      case "--replay-scope":
        opts.replayScope = args[++i] as ReplayScope;
        break;
      case "--help":
      case "-h":
        opts.help = true;
...
interface CLIVerboseResult {
  success: boolean;
  governanceReplayBlocked?: boolean;
  governanceMemoryWritten?: boolean;
  adrDraftCreated?: boolean;
  freezeSnapshotCreated?: boolean;
...
async function main(): Promise<void> {
  ...
  let input: CognitiveReviewInputV2;
  try {
    const raw = readFileSync(opts.input, "utf-8");
    input = JSON.parse(raw) as CognitiveReviewInputV2;
  } catch (err) {
    const errorResult: CLIVerboseResult = {
      success: false,
      error: {
  ...
  if (opts.enableReplay && memoryFile) {
    ...
    for (const entry of memoryFile.entries) {
      try { memoryStore.appendGovernanceMemoryEntry(entry); } catch { /* skip dupes */ }
    }
    ...
    const driftWarnings: Array<{ type: string; severity: string; description: string; detail?: string }> = [];
  ...
  if (opts.timeout) providerConfig.timeoutMs = opts.timeout;
  if (opts.model) providerConfig.model = opts.model;

  if (useLocalDecision) {
    // Local-only decision: bypass external provider
    const verboseResult: CLIVerboseResult & { result: unknown } = {
      success: true,
  ...
  const provider = opts.provider === "openai"
    ? new OpenAIReviewerProvider(providerConfig as Partial<import("./review-types.js").OpenAIReviewerConfig>)
  ...
  try {
    ...
    if (opts.writeMemory && memoryFile) {
      ...
      for (const entry of memoryFile.entries) {
        try { memoryStore.appendGovernanceMemoryEntry(entry); } catch { /* skip */ }
      }

      const entry: GovernanceMemoryEntry = memoryStore.createGovernanceMemoryEntry({

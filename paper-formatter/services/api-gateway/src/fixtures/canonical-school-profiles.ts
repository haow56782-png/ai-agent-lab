...
interface CanonicalSchoolRegistryEntry {
  schoolId: string;
  name: string;
  aliases: string[];
}
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (Array.isArray(parsed)) {
        return parsed as CanonicalSchoolRegistryEntry[];
      }
    } catch {
      continue;
    }
...
export const CANONICAL_PROFILE_SEEDS: CanonicalProfileSeed[] = [
  {
    schoolId: "thu",
    name: "清华大学",
    faculty: "计算机科学与技术系",
...
const registryBySchoolId = new Map(
  loadCanonicalSchoolRegistry().map((entry) => [entry.schoolId, entry]),
);
...
export function resolveCanonicalProfileSeed(name: string): CanonicalProfileSeed | null {
  const normalized = normalizeSchoolName(name);
  for (const seed of CANONICAL_PROFILE_SEEDS) {
    if (normalizeSchoolName(seed.schoolId) === normalized) return seed;
    if (normalizeSchoolName(seed.name) === normalized) return seed;
    if (seed.aliases.some((alias) => normalizeSchoolName(alias) === normalized)) return seed;
  }
  return null;
}

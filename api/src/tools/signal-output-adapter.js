...
/* ── Direction mapping ──────────────────────────────────── */
function predictionToDirection(prediction) {
    const p = prediction.toUpperCase();
    if (p.includes("UP") || p.includes("HIGH"))
        return "up";
    if (p.includes("DOWN") || p.includes("LOW"))
        return "down";
...
function recommendationToAction(recommendation) {
    ...
    if (r === "SKIP")
        return "avoid";
    if (r === "CAUTION" || r === "REDUCE_SIZE")
        return "review";
    if (r === "STOP_SESSION" || r === "WAIT")
        return "wait";
    return "unknown";
}
/* ── Warnings → failure modes ───────────────────────────── */
function warningsToFailureModes(warnings) {
    const modes = [];
...
export function normalizeFromDomainSignal(signal, toolName, traceId) {
    ...
    const factors = [
        ...
        ...(signal.expectedValue !== undefined
            ? [{ name: "expected_value", value: signal.expectedValue, weight: 0.7, explanation: "Expected return multiplier" }]
            : []),
    ];
    const risk = {
...
export function normalizeFromDecisionOutput(decision, toolName, traceId) {
    ...
    const evidence = [
        ...
        ...(decision.takeProfit !== undefined
            ? [{ key: "take_profit", value: decision.takeProfit, source: "decision_engine", weight: 0.5 }]
            : []),
    ];
    const factors = [
...
export function normalizeFromPredictionResult(result, toolName, traceId) {
    ...
    const factors = result.factors.map((f, i) => ({
        name: `factor_${i + 1}`,
        value: f,
        weight: 0.5,
        explanation: f,
...
export function normalizeFromGameMetrics(metrics, toolName, traceId) {
...
export function normalizeFromSimulation(params, toolName) {
    const evidence = [
...
export function toSignalOutputContract(input, toolName, traceId) {
    switch (input.type) {
        case "domain_signal":
...
export function inferAction(input) {
    if (input.confidence < 0.2 || input.riskLevel === "EXTREME")
        return "avoid";
    if (input.confidence < 0.4 || input.riskLevel === "HIGH")
        return "wait";
    if (input.confidence < 0.6 || input.riskLevel === "MEDIUM")
        return "review";
 ...
 */
export function shouldRequireReview(signal) {
    return (signal.risk.risk_level === "HIGH" ||
        signal.risk.risk_level === "EXTREME" ||
        signal.audit.reviewer_required);
}
//# sourceMappingURL=signal-output-adapter.js.map
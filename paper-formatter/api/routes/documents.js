...
documentRoutes.post("/", upload.single("file"), async (req, res, next) => {
    try {
        if (!req.file)
            throw createError(400, ERROR_CODES.VALIDATION_ERROR, "file is required");
        ...
        if (req.file.buffer.slice(0, 8).equals(OLE2_MAGIC)) {
            throw createError(400, ERROR_CODES.UNSUPPORTED_FORMAT, "文件是旧版 .doc 格式（WPS 兼容模式），不支持直接排版。请用 WPS/Word 打开后另存为 .docx 格式再试。");
        }
...
documentRoutes.get("/storage", async (req, res, next) => {
    try {
        ...
        if (!bucket || !key)
            throw createError(400, ERROR_CODES.VALIDATION_ERROR, "bucket and path query params required");
        const bucketMap = {
        ...
        if (!mapped)
            throw createError(404, ERROR_CODES.NOT_FOUND, "Unknown storage bucket");

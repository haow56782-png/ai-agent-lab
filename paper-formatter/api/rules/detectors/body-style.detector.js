...
export function detectBodyStyle(ctx) {
    const thresholds = resolveBodyStyleThresholds(ctx.profile);
    const paragraphs = (ctx.paragraphs || []).filter((paragraph) => cleanText(paragraph?.text).length > 0 && !paragraph?.is_heading);
    if (paragraphs.length === 0)
        return [];
    const fonts = new Set();
    const sizes = new Set();
    const lineSpacings = new Set();
    const firstLineIndents = new Set();
    for (const paragraph of paragraphs.slice(0, 40)) {
        for (const run of paragraph.runs || []) {
            if (run?.name)
                fonts.add(String(run.name));
            if (run?.size_pt)
                sizes.add(Math.round(Number(run.size_pt)));
        }
        if (paragraph?.spacing?.line_spacing)
            lineSpacings.add(Number(paragraph.spacing.line_spacing));
        if (paragraph?.indent?.first_line_cm)
            firstLineIndents.add(Number(paragraph.indent.first_line_cm));
    }
    const detections = [];
...
export function detectHeadingSpacing(ctx) {
    const thresholds = resolveBodyStyleThresholds(ctx.profile);
    const levelOneParagraph = (ctx.paragraphs || []).find((paragraph) => paragraph?.is_heading && Number(paragraph?.heading_level || 0) === 1);
    if (!levelOneParagraph)
        return [];
    const before = Number(levelOneParagraph?.spacing?.before_pt || 0);
    const after = Number(levelOneParagraph?.spacing?.after_pt || 0);
    if (Math.abs(before - thresholds.headingBeforePt) <= 6 && Math.abs(after - thresholds.headingAfterPt) <= 6)
        return [];
...
export const bodyStyleDetector = {
    ruleId: BODY_STYLE_RULE_ID,
    label: BODY_STYLE_LABEL,
    group: "样式",
    severity: "P2",
...
export const headingSpacingDetector = {
    ruleId: HEADING_SPACING_RULE_ID,
    label: HEADING_SPACING_LABEL,
    group: "样式",
    severity: "P2",

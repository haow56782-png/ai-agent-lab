...
function findParagraphByIndex(paragraphs, paragraphIndex) {
    return paragraphs[paragraphIndex] ?? paragraphs.find((paragraph) => Number(paragraph?.index || -1) === paragraphIndex) ?? null;
}
...
function isCaptionStyleTemplateFinding(detection) {
...
function aggregateCaptionStyleDetections(detections) {
    const output = [];
...
export function detectCaptionPosition(ctx) {
    const detections = [];
    ...
    for (const relation of relations) {
        ...
        const structureItem = structureItems.find((item) => item?.type === itemType && `caption:${item.type}:${Number(item?.index ?? -1)}` === relation.fromObjectId);
        if (!structureItem)
            continue;
...
export const captionPositionDetector = {
    ruleId: FIGURE_CAPTION_POSITION_RULE_ID,
    label: FIGURE_CAPTION_POSITION_LABEL,
    group: "图表 & 题注",
    severity: "P2",

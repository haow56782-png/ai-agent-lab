...
function findHierarchyIssue(headings: any[]): { heading: any; reason: string } | null {
  if (headings.length === 0) return { heading: { index: 0, text: "未检测到标题" }, reason: "文档未检测到明确标题层级" };
  const hasLevelOne = headings.some((heading) => Number(heading?.level || 0) === 1);
  if (!hasLevelOne) return { heading: headings[0], reason: "文档存在标题，但未检测到一级标题作为章节锚点" };
...
export function detectHeadingHierarchy(ctx: ParsedDocumentContext): RuleDetection[] {
...
export const headingHierarchyDetector: FormatRuleDetector = {
  ruleId: HEADING_HIERARCHY_RULE_ID,
  label: HEADING_HIERARCHY_LABEL,
  group: "样式",
  severity: "P2",

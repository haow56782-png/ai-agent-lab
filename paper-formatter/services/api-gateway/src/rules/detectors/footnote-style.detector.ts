...
function isAllowedFootnoteFont(value: unknown): boolean {
  const font = normalizeFontName(value);
  return font.includes("times") || font.includes("宋") || font.includes("仿宋") || font.includes("arial");
}

export function detectFootnoteStyleIssues(ctx: ParsedDocumentContext): RuleDetection[] {
  const footnoteParagraph = (ctx.paragraphs || []).find((paragraph: any) => /footnote|脚注/i.test(paragraph?.style || ""));
  if (!footnoteParagraph) return [];
  const runFonts = Array.isArray(footnoteParagraph.runs) ? footnoteParagraph.runs.map((run: any) => run?.name).filter(Boolean) : [];
  const runSizes = Array.isArray(footnoteParagraph.runs) ? footnoteParagraph.runs.map((run: any) => Number(run?.size_pt || 0)).filter(Boolean) : [];
  const hasDisallowedFont = runFonts.some((font: unknown) => !isAllowedFootnoteFont(font));
  const hasUnexpectedSize = runSizes.some((size: number) => size > 11.5 || size < 8.5);
  if (!hasDisallowedFont && !hasUnexpectedSize) return [];
...
export const footnoteStyleDetector: FormatRuleDetector = {
  ruleId: FOOTNOTE_STYLE_RULE_ID,
  label: FOOTNOTE_STYLE_LABEL,
  group: "样式",
  severity: "P2",

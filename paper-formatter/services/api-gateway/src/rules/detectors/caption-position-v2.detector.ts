...
import type {
  ...
  ObjectGraphNode,
  TableObject,
} from "../../parser/object-graph/types.js";
import type { RuleDetection } from "../rule-types.js";
import {
  FIGURE_CAPTION_POSITION_LABEL,
  FIGURE_CAPTION_POSITION_RULE_ID,
  TABLE_CAPTION_POSITION_LABEL,
  TABLE_CAPTION_POSITION_RULE_ID,
} from "./caption-position.detector.js";

const GROUP = "图表 & 题注";
export const FIGURE_CAPTION_POSITION_V2_RULE_ID = "CAPTION_POSITION_OBJECT_GRAPH_REVIEW";

export function detectCaptionPositionV2(graph: ObjectGraph): RuleDetection[] {
  ...
  for (const edge of captionedByEdges) {
    ...
    if (!target || !isCaptionNode(caption)) continue;
    const relationType = resolveCaptionRelationType(caption, target);
    if (relationType !== "caption_wrong_position") continue;
  ...
  for (const caption of graph.nodes.filter(isCaptionNode)) {
    if (captionIdsWithEdges.has(caption.objectId)) continue;
...
function createDetection(input: {
  caption: CaptionNode;
  target: ObjectGraphNode | null;
  relationId: string;
  relationType: "caption_wrong_position" | "caption_unbound";
  confidence: number;
}): RuleDetection {
...
function resolveCaptionRelationType(
  caption: CaptionNode,
  target: ObjectGraphNode,
): "caption_belongs_to_figure" | "caption_belongs_to_table" | "caption_wrong_position" {
  if (caption.captionKind === "figure") {
    if (!isFigureObject(target)) return "caption_wrong_position";
    return caption.paragraphIndex > target.anchorParagraphIndex
      ? "caption_belongs_to_figure"
      : "caption_wrong_position";
  }
  if (!isTableObject(target)) return "caption_wrong_position";
...
function isFigureObject(node: ObjectGraphNode): node is FigureObject {
  return node.kind === "figure";
}

function isTableObject(node: ObjectGraphNode): node is TableObject {

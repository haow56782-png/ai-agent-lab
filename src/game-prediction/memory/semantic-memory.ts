...
export function buildSemanticMemory(
  ...
  const riskEscalationLevel = computeRiskEscalation(
    riskScore,
    profile.tiltCount,
    profile.totalStopSession,
    repeatedTiltEscalation,
  );

  // Persistent warnings
  const persistentWarnings: string[] = [];
  if (riskScore > 0.6) {
 ...
 */
function detectTiltEscalation(events: MemoryEvent[], profile: PlayerProfile): boolean {
  // Group tilt events by session
  const tiltBySession = new Map<string, number>();
  ...
  if (sessionIds.length < 4) return false;
 ...
 */
function computeRiskEscalation(
  riskScore: number,
  tiltCount: number,
  stopSessionCount: number,
  tiltEscalating: boolean,
): "none" | "monitor" | "elevated" | "high" | "critical" {
  if (riskScore > 0.8 || (riskScore > 0.6 && tiltEscalating)) return "critical";
  if (riskScore > 0.6 || stopSessionCount > 3) return "high";
  if (riskScore > 0.4 || tiltCount > 3) return "elevated";
  if (riskScore > 0.2 || tiltCount > 0) return "monitor";
  return "none";
}

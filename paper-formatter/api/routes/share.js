...
function prettySchoolName(schoolId) {
    if (!schoolId)
        return "";
    return schoolId
        .replace(/[_-]+/g, " ")
...
function extractTopIssues(result) {
    const ruleDetails = Array.isArray(result?.ruleDetails) ? result.ruleDetails : [];
    const issues = ruleDetails.flatMap((group) => Array.isArray(group?.items)
    ...
    if (completedSteps.length > 0) {
        return completedSteps.slice(0, 5).map((step) => ({
...
async function resolveShareContext(legacyDocId, checkResultLegacyDocId) {
...
async function buildSharePayload(req, legacyDocId, checkResultLegacyDocId, shareId) {
    ...
    const result = (sourceJob?.result_json || {});
...
function renderSharePage(shareId, payload) {
...
function renderOgSvg(shareId, payload, poster = false) {
...
shareRoutes.post("/report", async (req, res, next) => {
    try {
        ...
        const { payload, sourceJobId, docId, profileId } = await buildSharePayload(req, command.legacyDocId, command.checkResultLegacyDocId, shareId);

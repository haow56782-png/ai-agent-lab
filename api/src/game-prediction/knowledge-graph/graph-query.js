 ...
 *  ============================================================ */
export async function executeGraphQuery(store, query) {
    switch (query.type) {
...
async function queryPlayerRiskPatterns(store, query) {
    ...
    for (const node of patternNodes) {
        ...
        if (playerId) {
            ...
            const connectedToPlayer = edges.some((e) => e.sourceId === playerNodeId || e.targetId === playerNodeId);
        ...
        const relatedStrategies = new Set();
        ...
        results.push({
            ...
            trend: frequency > 3 ? "increasing" : frequency > 0 ? "stable" : "decreasing",
        });
    }
    return results.sort((a, b) => b.frequency - a.frequency).slice(0, query.limit ?? 20);
}
/* ─── STRATEGY_RELIABILITY ─── */
async function queryStrategyReliability(store, query) {
    const allNodes = [...store._getAllNodes().values()];
    const strategyNodes = allNodes.filter((n) => n.type === "STRATEGY");
    const playerId = query.playerId ?? "";
    const results = [];
    for (const node of strategyNodes) {
        if (query.strategyName && node.properties.strategyName !== query.strategyName)
            continue;
        // Check player association
        if (playerId) {
            const edges = await store.getEdgesForNode(node.id);
            const playerNodeId = `PLAYER::${playerId}`;
            if (!edges.some((e) => e.sourceId === playerNodeId || e.targetId === playerNodeId))
                continue;
        }
        const edges = await store.getEdgesForNode(node.id);
...
/* ─── STOP_SESSION_PATH ─── */
async function queryStopSessionPath(store, query) {
    const playerId = query.playerId ?? "";
    const playerNodeId = `PLAYER::${playerId}`;
    ...
    for (const edge of stopEdges) {
        ...
        if (sourceNode) {
            const preEdges = allEdges.filter((e) => (e.targetId === sourceNode.id || e.sourceId === sourceNode.id) &&
...
/* ─── GAME_BEHAVIOR_RISKS ─── */
async function queryGameBehaviorRisks(store, query) {
    const allEdges = [...store._getAllEdges().values()];
    const allNodes = [...store._getAllNodes().values()];
    ...
    if (gameType && allNodes.some((n) => n.id === gameNodeId)) {
        ...
        const playedEdges = allEdges.filter((e) => e.type === "PLAYED_GAME" && e.targetId === gameNodeId);
        const sessionIds = new Set(playedEdges.map((e) => e.sourceId));
        // Find behavior events in those sessions
        for (const sessionId of sessionIds) {
            const sessionEdges = allEdges.filter((e) => e.type === "TRIGGERED_BEHAVIOR" && e.sourceId === sessionId);
...
async function queryLearningDriftDowngrade(store, query) {
    const allEdges = [...store._getAllEdges().values()];
    const allNodes = [...store._getAllNodes().values()];
    ...
    for (const stratNode of strategyNodes) {
        if (query.strategyName && stratNode.properties.strategyName !== query.strategyName)
            continue;
        ...
        const signalEdges = allEdges.filter((e) => e.type === "UPDATED_CONFIDENCE" &&
...
/* ─── DEBATE_MOST_OPPOSED ─── */
async function queryDebateMostOpposed(store, query) {
    const allEdges = [...store._getAllEdges().values()];
    const allNodes = [...store._getAllNodes().values()];
    ...
    for (const debateNode of debateNodes) {
        ...
        for (const edge of edges) {
            if (edge.type === "REVIEWED_BY_DEBATE") {
                ...
                const playerEdges = allEdges.filter((e) => e.type === "USED_STRATEGY" && e.targetId === edge.targetId);
    ...
    const results = [...strategyMap.entries()]

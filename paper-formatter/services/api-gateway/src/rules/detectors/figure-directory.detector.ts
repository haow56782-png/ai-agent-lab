...
function hasFigureDirectory(paragraphs: any[]): boolean {
  return paragraphs.some((paragraph: any) => /图目录|list\s+of\s+figures/i.test(String(paragraph?.text || "")));
}

export function detectFigureDirectoryReview(ctx: ParsedDocumentContext): RuleDetection[] {
  const figureCaptions = (ctx.structureItems || []).filter((item: any) => item?.type === "figure_caption");
  if (figureCaptions.length < 2) return [];
  if (hasFigureDirectory(ctx.paragraphs || [])) return [];
...
export const figureDirectoryDetector: FormatRuleDetector = {
  ruleId: FIGURE_DIRECTORY_RULE_ID,
  label: FIGURE_DIRECTORY_LABEL,
  group: "目录 & 域",
  severity: "P2",

...
import { CANONICAL_PROFILE_SEEDS, resolveCanonicalProfileSeed, } from "../fixtures/canonical-school-profiles.js";
...
function isInvalidDetectedSchoolName(name) {
...
export async function searchProfiles(params) {
    ...
    const result = await query(`SELECT * FROM school_profiles ${where} ORDER BY school_id, version DESC`, values);
...
export async function getProfileRules(profileId) {
    ...
    const result = await query("SELECT school_id, rules_json, style_map FROM school_profiles WHERE school_id = $1 AND source_type <> $2", [profileId, ARCHIVED_DETECTED_SOURCE_TYPE]);
...
export async function createProfile(record) {
    const result = await query(`INSERT INTO school_profiles (school_id, name, version, effective_from, faculty, major, rules_json, style_map, source_type)
...
export async function findProfileByName(name) {
    const result = await query(`SELECT * FROM school_profiles
      WHERE (name ILIKE $1 OR $1 ILIKE '%' || name || '%')
        AND source_type <> $2
      LIMIT 1`, [name, ARCHIVED_DETECTED_SOURCE_TYPE]);
    return result.rows[0] || null;
}
function hasRulePayload(profile) {
...
async function hydrateProfileRulesFromRuleTable(profile) {
    if (!profile)
        return profile;
...
function rankProfileMatch(profile) {
    let score = 0;
    if (hasRulePayload(profile))
        score += 100;
    if (profile.source_type === "seed" || profile.source_type === "official")
        score += 40;
    if (profile.source_type === "detected")
        score -= 20;
...
function collapseProfiles(profiles) {
...
export async function ensureCanonicalProfile(seed) {
    ...
    const shouldRefreshRulePayload = hasRulePayload(existing) && (JSON.stringify(existing.rules_json || []) !== JSON.stringify(seed.rulesJson)
    ...
    if (!hasRulePayload(existing) || shouldNormalizeSourceType || shouldRefreshRulePayload) {
        const result = await query(`UPDATE school_profiles
          SET name = $2,
              version = $3,
              effective_from = $4,
...
export async function findBestProfileByName(name) {
    ...
    const result = await query(`SELECT * FROM school_profiles
      WHERE (name ILIKE $1 OR $1 ILIKE '%' || name || '%')
        AND source_type <> $2
      ORDER BY upload_count DESC, updated_at DESC`, [name, ARCHIVED_DETECTED_SOURCE_TYPE]);
    if (result.rows.length === 0)
        return null;
    return result.rows.sort((left, right) => rankProfileMatch(right) - rankProfileMatch(left))[0] || null;
}
export async function findProfileById(schoolId) {
    const result = await query("SELECT * FROM school_profiles WHERE school_id = $1 AND source_type <> $2", [schoolId, ARCHIVED_DETECTED_SOURCE_TYPE]);
    return result.rows[0] || null;
}
export async function incrementUploadCount(schoolId) {
    await query(`UPDATE school_profiles SET upload_count = upload_count + 1 WHERE school_id = $1`, [schoolId]);
}
function isEmptyRulePayload(value) {
    return !Array.isArray(value) || value.length === 0;
}
export async function ensureSeedProfileSamples() {
    for (const sample of PROFILE_RULE_SAMPLES) {
        const existing = await query(`SELECT school_id, rules_json, style_map
         FROM school_profiles
        WHERE school_id = $1
        ORDER BY effective_from DESC
        LIMIT 1`, [sample.schoolId]);
        if (existing.rows.length === 0) {
            await query(`INSERT INTO school_profiles (school_id, name, version, effective_from, faculty, rules_json, style_map, source_type)
        ...
        await query(`UPDATE school_profiles
          SET rules_json = CASE WHEN rules_json = '[]'::jsonb THEN $2::jsonb ELSE rules_json END,
              ...
              updated_at = NOW()
        WHERE school_id = $1`, [sample.schoolId, JSON.stringify(sample.rulesJson), JSON.stringify(sample.styleMap)]);
    }
}
...
export async function normalizeCanonicalProfileSourceTypes() {
    let normalized = 0;
    for (const seed of CANONICAL_PROFILE_SEEDS) {
...
export async function migrateLegacyDetectedProfiles() {
    const client = await getPool().connect();
    try {
        await client.query("BEGIN");
        const legacyProfiles = await client.query(`SELECT * FROM school_profiles
        ...
        for (const legacy of legacyProfiles.rows) {
            if (isInvalidDetectedSchoolName(legacy.name)) {
                await client.query(`UPDATE school_profiles
              SET source_type = $2,
                  upload_count = 0,
                  updated_at = NOW()
            WHERE school_id = $1`, [legacy.school_id, ARCHIVED_DETECTED_SOURCE_TYPE]);
                migratedProfiles += 1;
                continue;
            }
            ...
            const canonicalResult = await client.query(`SELECT * FROM school_profiles
          WHERE school_id = $1
          LIMIT 1`, [canonicalSeed.schoolId]);
            if (canonicalResult.rows.length === 0)
                continue;
            const canonical = canonicalResult.rows[0];
            const docLinks = await client.query(`SELECT doc_id, confidence
           FROM document_profiles
          WHERE school_id = $1`, [legacy.school_id]);
            for (const link of docLinks.rows) {
                if (!link.doc_id)
                    continue;
                const inserted = await client.query(`INSERT INTO document_profiles (doc_id, school_id, profile_id, confidence)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (doc_id, school_id) DO NOTHING`, [link.doc_id, canonical.school_id, canonical.school_id, link.confidence]);
            ...
            if ((legacy.upload_count || 0) > 0) {
              ...
              SET upload_count = upload_count + $2,
                  updated_at = NOW()
            WHERE school_id = $1`, [canonical.school_id, legacy.upload_count]);
            }
            await client.query(`UPDATE school_profiles
            SET source_type = $2,
                upload_count = 0,

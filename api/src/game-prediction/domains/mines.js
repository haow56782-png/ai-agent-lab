...
export function getRiskProfile() {
    return {
        ...
        description: "Mines is a high-volatility game with combinatorial probability. "
            + "Each pick is independent. A mine can appear on the first pick "
            + "regardless of how many squares remain.",
    };
}
export function parseMinesInput(input) {
    ...
    if (!Number.isInteger(minesCount) || minesCount < 1 || minesCount >= totalSquares) {
        throw new Error(`Invalid minesCount: ${minesCount}. Must be 1–${totalSquares - 1}.`);
    }
    if (!Number.isInteger(picksCount) || picksCount < 1 || picksCount > totalSquares - minesCount) {
        throw new Error(`Invalid picksCount: ${picksCount}. Must be 1–${totalSquares - minesCount}.`);
    }
...
/* ─── Recommendation ─── */
export function generateRecommendation(parsed, probability) {
    const { minesCount, picksCount } = parsed;
...
export function getExplanation(parsed, probability) {
    ...
    if (minesCount <= 3 && picksCount <= 2) {
        parts.push("Low mine count with few picks gives reasonable survival odds. However, a mine can appear on the first pick regardless.");
    }
    else if (minesCount >= 10) {
        parts.push("High mine density means very low survival probability after even one pick.");
    }
    else {
        parts.push("Combinatorial probability decreases rapidly with each additional pick.");
    }
    return parts.join(" ");
}
/* ── Disclaimers ── */

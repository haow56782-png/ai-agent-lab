...
import type { GameInput } from "../domains/types.js";
...
const STRATEGY_REGISTRY: Record<string, Record<string, import("../strategies/types.js").Strategy>> = {
  DICE: diceStrategies,
  CRASH: crashStrategies,
  MINES: minesStrategies,
};

const RISK_TO_STRATEGY: Record<GameType, Record<RiskPreference, string>> = {
 ...
 */
export function selectStrategyName(gameType: GameType, riskPreference: RiskPreference): string {
 ...
 */
export function getStrategy(gameType: string, strategyName: string): import("../strategies/types.js").Strategy {
...
export function applyPolicy(
  ...
  const warnings: string[] = [...strategyResult.warnings, ...riskAssessment.warnings];
  ...
  if (
    input.domainSignal.recommendation === "SKIP" &&
  ...
  const overbetSevere = riskAssessment.warnings.some(
    (w) => w.includes("Overbet") && (w.includes("severe") || w.includes("critical")),
  );
  if (overbetSevere) {
  ...
  const recommendedBetSize = Math.round(
    Math.min(
      strategyResult.recommendedBetSizeFraction * input.bankroll,
      riskAssessment.recommendedMaxBet,
    ) * 100,
  ) / 100;
...
export function getDefaultAssumptions(): string[] {
  return [

...
function normalizeRuleId(rule: Record<string, any>, fallbackIndex: number): string {
  return String(rule.ruleId || rule.rule_id || `rule_${String(fallbackIndex + 1).padStart(3, "0")}`);
}

function normalizeRuleName(rule: Record<string, any>, ruleId: string): string {
  return String(rule.ruleName || rule.rule_name || rule.label || rule.description || ruleId);
}

function buildExpectedFormat(rule: Record<string, any>): Record<string, unknown> {
...
function buildCondition(rule: Record<string, any>): Record<string, unknown> {
  return {
...
function numericCategoryCode(rule: Record<string, any>, fallbackIndex: number): number {
  const value = Number.parseInt(String(rule.categoryCode || "").replace(/\D/g, ""), 10);
  if (Number.isFinite(value)) return value * 1000 + fallbackIndex;
  return 999_000 + fallbackIndex;
}

function normalizeSchoolRule(input: RuleSetSyncInput, rule: Record<string, any>, index: number, cacheKind: "rule" | "style"): SchoolRuleRow {
...
export function buildSchoolRuleRows(input: RuleSetSyncInput): SchoolRuleRow[] {
  return [
...
export async function upsertRuleSetFromProfile(input: RuleSetSyncInput): Promise<{
  ruleSetId: string;
  ruleCount: number;
}> {
  ...
  try {
    ...
    const ruleSetResult = await client.query<{ rule_set_id: string }>(
      `INSERT INTO school_rule_sets (
...
export async function createRuleSnapshot(input: RuleSnapshotInput): Promise<string> {
  const result = await query<{ snapshot_id: string }>(
...
export async function getLatestRuleSetForSchool(schoolId: string): Promise<{
  ruleSetId: string;
  schoolId: string;
  version: string;
} | null> {
  const result = await query<{ rule_set_id: string; school_id: string; version: string }>(
    `SELECT rule_set_id, school_id, version
...
export async function getProfileRulePayloadFromRuleTable(schoolId: string): Promise<{
  ruleSetId: string;
  rulesJson: Record<string, any>[];
  styleMap: Record<string, any>[];
} | null> {
  ...
  const result = await query<{ cache_kind: "rule" | "style"; raw_rule_json: Record<string, any> }>(
    `SELECT cache_kind, raw_rule_json
       FROM school_rules
      WHERE rule_set_id = $1
...
export async function listRulesForSnapshot(params: {
  ...
  const result = await query<{ rule_id: string; raw_rule_json: Record<string, unknown> }>(
    `SELECT rule_id, raw_rule_json
       FROM school_rules
      WHERE rule_set_id = $1
        AND rule_id = ANY($2::varchar[])`,
    [ruleSet.ruleSetId, uniqueRuleIds],
  );
...
function getCanonicalMappingFromFinding(finding: FindingContract): Record<string, unknown> | undefined {
...
function buildFallbackRuleSnapshotPayload(finding: FindingContract): Record<string, unknown> {

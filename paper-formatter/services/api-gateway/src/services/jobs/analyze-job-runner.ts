...
function getRuleStatus(ruleHit: JobRuleHitItem | [string, "pass" | "warn"]): "pass" | "warn" | "fail" {
  return Array.isArray(ruleHit) ? ruleHit[1] : ruleHit.status;
}

export async function processAnalyzeJob(jobId: string, documentRecord: documentRepo.DocumentRecord, profileId?: string) {
  const hasNoProfile = !profileId;
  const profileRules = profileId ? await profileRepo.getProfileRules(profileId).catch(() => null) : null;
  const updateAnalyzeJob = (patch: Record<string, any>) =>
    jobRepo.updateJob(jobId, patch).catch((error) =>
      console.error(`[analyze] Failed to update job ${jobId}:`, error.message)
    );
  ...
  try {
    ...
    } catch (objectGraphError: any) {
      console.warn("[analyze] Object graph extraction failed, falling back to legacy detectors:", objectGraphError.message);
    }

    const coverItem = structureItems.find((structureItem: any) => structureItem.type === "cover");
    const abstractItem = structureItems.find((structureItem: any) => structureItem.type === "abstract");
    const tocItem = structureItems.find((structureItem: any) => structureItem.type === "toc");
    const figureCaptions = structureItems.filter((structureItem: any) => structureItem.type === "figure_caption");
    const tableCaptions = structureItems.filter((structureItem: any) => structureItem.type === "table_caption");
    const referenceEntries = structureItems.filter((structureItem: any) => structureItem.type === "reference_entry");

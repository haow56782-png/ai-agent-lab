...
import { parseAutoCreateSchoolCommand, parseDetectSchoolCommand, } from "../dto/document-requests.js";
...
const templateUpload = multer({
    ...
    fileFilter: (_req, file, cb) => {
        ...
        if (ext === "docx" || ext === "pdf") {
            ...
            cb(createError(422, ERROR_CODES.UNSUPPORTED_FORMAT, `Unsupported format: .${ext}`));
        }
    },
});
...
profileRoutes.post("/detect", async (req, res, next) => {
    try {
        ...
        if (!doc)
            throw createError(404, ERROR_CODES.NOT_FOUND, "Document not found");
    ...
    try {
        ...
        if (existing) {
            if (command.legacyDocId) {
                await query(`INSERT INTO document_profiles (doc_id, school_id, profile_id) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, [command.legacyDocId, existing.school_id, existing.school_id]);
        ...
        if (command.legacyDocId) {
            await query(`INSERT INTO document_profiles (doc_id, school_id, profile_id) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, [command.legacyDocId, schoolId, schoolId]);

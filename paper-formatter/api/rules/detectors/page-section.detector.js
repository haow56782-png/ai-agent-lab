...
function hasSectionBreakIssue(ctx) {
    const levelOneHeadings = (ctx.headings || []).filter((heading) => Number(heading?.level || 0) === 1);
    ...
    if (levelOneHeadings.length < 2)
        return null;
    const problematicHeading = levelOneHeadings.slice(1).find((heading) => {
        const paragraph = ctx.paragraphs[Number(heading?.index || 0)];
        return !paragraph?.page_break_before;
    });
    if (!problematicHeading)
        return null;
    return {
...
export function detectPageSectionReview(ctx) {
...
export const pageSectionDetector = {
    ruleId: PAGE_SECTION_RULE_ID,
    label: PAGE_SECTION_LABEL,
    group: "分节 & 页码",
    severity: "P2",

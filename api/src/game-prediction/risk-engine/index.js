 ...
 *  ============================================================ */
import { checkBankroll, checkOverbet, shouldStopSession, getRecommendedMaxBet, } from "./bankroll.js";
/**
...
export function assessRisk(input) {
    ...
    // Bankroll check
    const bankrollStatus = checkBankroll(input.bankroll, input.betSize, input.riskPreference, consecutiveLosses, input.domainSignal.probability);
    if (bankrollStatus.isOverbet) {
        warnings.push(`Overbet: bet size ${input.betSize} is ${(bankrollStatus.exposureRatio * 100).toFixed(0)}% of bankroll (${input.riskPreference} limit: ${(bankrollStatus.overbetLevel === "critical" ? ">" : "")}${bankrollStatus.overbetLevel === "critical" ? "20" : bankrollStatus.overbetLevel === "severe" ? "10" : "5"}%).`);
    }
    ...
    if (bankrollStatus.riskOfRuin > 0.3) {
        warnings.push(`High risk of ruin: ${(bankrollStatus.riskOfRuin * 100).toFixed(0)}% chance of losing entire bankroll at current bet size.`);
    }
    // Session stop check
    const stopCheck = shouldStopSession(consecutiveLosses, input.riskPreference, input.maxLoss, totalLoss);
    if (stopCheck.stop) {
        return {
            passed: false,
...
/* ─── Helpers ─── */
function computeTotalLoss(input) {
    if (!input.recentResults || input.recentResults.length === 0)
        return 0;
    const losses = input.recentResults.filter((r) => r === "loss").length;
    return losses * input.betSize;
}
function computeConsecutiveLosses(input) {
    if (!input.recentResults || input.recentResults.length === 0)
        return 0;
    let count = 0;
    for (let i = input.recentResults.length - 1; i >= 0; i--) {
        if (input.recentResults[i] === "loss")
            count++;
        else
            break;
    }

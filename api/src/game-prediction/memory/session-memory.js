 ...
 */
export function buildSessionMemory(sessionId, playerId, events, currentBankroll, startingBankroll) {
    ...
    const recentResults = predictionResults
        .slice(-20)
        .map((e) => e.data.result);
    // Find active strategy from most recent prediction
    const sortedPredictions = [...predictionResults].sort((a, b) => b.timestamp.localeCompare(a.timestamp));
    const activeStrategy = sortedPredictions.length > 0
        ? sortedPredictions[0].data.strategyName ?? "unknown"
        : "unknown";
    // Current streak
    let streakType = "win";
    let streakCount = 0;
    ...
    for (const e of interventionEvents) {
        const ws = e.data.warnings;
        if (ws)
            sessionWarnings.push(...ws);
    }
    // Duration

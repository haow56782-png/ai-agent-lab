 ...
 */
export function createPrior(confidence, strength = 10) {
 ...
 */
export function updatePosterior(prior, wins, losses, overconfidencePenalty = 0) {
...
export function posteriorMean(dist) {
    return dist.alpha / (dist.alpha + dist.beta);
}
/** Mode of Beta distribution: (α-1) / (α+β-2) for α,β > 1 */
export function posteriorMode(dist) {
    if (dist.alpha <= 1 || dist.beta <= 1)
        return posteriorMean(dist);
    return (dist.alpha - 1) / (dist.alpha + dist.beta - 2);
}
/** Variance of Beta distribution */
export function posteriorVariance(dist) {
 ...
 */
export function computeOverconfidencePenalty(confidence, wasCorrect) {
    if (wasCorrect)
        return 0;
    if (confidence <= 0.5)
        return 0;
 ...
 */
export function suppressStreakBias(consecutiveErrors, rawPenalty) {
 ...
 */
export function widenUncertainty(dist) {
 ...
 */
export function bayesianCalibratedConfidence(currentConfidence, priorStrength, historicalWins, historicalLosses, wasCorrect, consecutiveErrors) {

...
async function loadDesignTokens(root, fs) {
...
registerTool({
    ...
    if (category === "all" || category === "typography") {
        sections.push(`## Typography (${Object.keys(tokens.typography).length} tokens)\n` +
            Object.entries(tokens.typography)
                .map(([k, v]) => `  ${k}: ${v}`)
                .join("\n"));
    }
    if (category === "all" || category === "spacing") {
        sections.push(`## Spacing (${Object.keys(tokens.spacing).length} tokens)\n` +
...
registerTool({
    ...
    description: "Generate VIB-themed CSS for a component using design tokens",
    parameters: {
        type: "object",
        properties: {
            component: {

 ...
 */
export function adjustConfidence(params: ConfidenceAdjustmentParams): number {
  ...
  const posterior = updatePosterior(
    prior,
    params.historicalWins + winContribution,
    params.historicalLosses + lossContribution,
    penalty,
  );
 ...
 */
export function computePredictionError(predictedProbability: number, actualResult: "win" | "loss"): number {
 ...
 */
export function computeBetFractionAdjustment(
  brierScore: number,

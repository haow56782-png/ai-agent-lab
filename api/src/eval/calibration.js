...
export class CalibrationReporter {
    ...
    observe(signalId, actualOutcome) {
    ...
    generateReport() {
        ...
        // Overall metrics
        const overallAccuracy = observedSignals > 0
            ? observed.filter((r) => r.accurate).length / observedSignals
            : 0;
        const avgConfidence = observedSignals > 0
        ...
        const byConfidenceBucket = Array.from(buckets.entries())
        ...
        return {
            ...
            calibrationError: observedSignals > 0
...
export function formatCalibrationReport(report) {
    ...
    if (report.byProvider.length > 0) {
        ...
        for (const p of report.byProvider) {
            lines.push(`  ${p.provider.padEnd(16)} ${p.signals} signals, ` +
    ...
    if (report.byConfidenceBucket.length > 0) {
        ...
        for (const b of report.byConfidenceBucket) {
            lines.push(`  conf ${b.bucket.padEnd(7)} ${b.count} samples, ${(b.accuracy * 100).toFixed(0)}% acc`);
        }
    }
    return lines.join("\n");
}

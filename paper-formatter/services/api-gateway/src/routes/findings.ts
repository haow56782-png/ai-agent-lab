...
import {
  ...
  toFindingListQuery,
} from "../dto/finding-document-requests.js";
...
function requireActor(body: Partial<FindingDispositionRequest>): { actorId: string; actorRole: ActorRole } {
...
findingRoutes.post("/:findingId/accept", async (req, res, next) => {
  try {
    const actor = requireActor(req.body as FindingDispositionRequest);
    ...
    if (!finding) throw createError(404, ERROR_CODES.NOT_FOUND, "Finding not found");
    res.json(finding);
  } catch (err) {
    next(err);
  }
...
findingRoutes.post("/:findingId/reject", async (req, res, next) => {
  try {
    const body = req.body as FindingDispositionRequest;
...
findingRoutes.post("/:findingId/self-edit", async (req, res, next) => {
  try {
    const body = req.body as FindingSelfEditRequest;

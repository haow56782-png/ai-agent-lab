 ...
 */
export function buildProfile(playerId, events, existingProfile) {
    ...
    for (const e of predictionEvents) {
        const s = e.data.strategyName;
        if (s)
            strategyAffinity[s] = (strategyAffinity[s] ?? 0) + 1;
    }
    ...
    for (const e of predictionEvents) {
        ...
        if (s && result) {
            if (!strategyReliability[s])
                strategyReliability[s] = 0;
    ...
    for (const e of calibrationEvents) {
        const c = e.data.calibratedConfidence;
        if (c != null)
            confidenceTrend.push(c);
    }
    // Brier score trend
    const brierScoreTrend = [];
    for (const e of calibrationEvents) {
        const b = e.data.brierScore;
        if (b != null)
            brierScoreTrend.push(b);
    }
    // Rolling accuracy trend
    const rollingAccuracyTrend = [];
    for (const e of calibrationEvents) {
        const a = e.data.rollingAccuracy;
        if (a != null)
            rollingAccuracyTrend.push(a);
    }
    // Behavior state history
    const behaviorStateHistory = events
        .filter((e) => e.type === "behavior_intervention")
        .map((e) => e.data.behaviorState)
        .filter((s) => s != null);
 ...
 */
function computeBankrollDiscipline(tiltCount, stopSessionCount, interventionCount, totalPredictions) {
    let score = 1.0;
 ...
 */
export async function getOrCreateProfile(playerId, events, existingProfile) {
    if (existingProfile) {

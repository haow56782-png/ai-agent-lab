...
function isBodyReference(text, prefix) {
    const compact = normalizeText(text);
    return compact.startsWith(`如${prefix}`) || compact.startsWith(`见${prefix}`);
}
export function detectCaptionKind(text) {
    const compact = normalizeText(text);
    if (!compact)
        return null;
...
function verticalDistance(a, b) {
    ...
    if (topA === null || bottomA === null || topB === null || bottomB === null)
        return null;
...
function flowDistance(a, b) {
    if (typeof a.flowOrder !== "number" || typeof b.flowOrder !== "number")
        return null;
    return Math.abs(a.flowOrder - b.flowOrder) * 100;
}
function paragraphDistance(a, b) {
    if (typeof a.paragraphIndex !== "number" || typeof b.paragraphIndex !== "number")
        return null;
    return Math.abs(a.paragraphIndex - b.paragraphIndex) * 100;
}
function samePageBoost(caption, target) {
    if (caption.page === undefined || target.page === undefined)
        return 0;
    return caption.page === target.page ? 0 : 10_000;
}
function relationPriority(caption, target) {
...
function isPreferredDirectionalMatch(caption, target, kind) {
...
function resolveRelationType(caption, target, kind) {
    const captionTop = objectTop(caption);
    const captionBottom = objectBottom(caption);
    const targetTop = objectTop(target);
    const targetBottom = objectBottom(target);
...
export function evaluateCaptionAnchors(objects) {
    const relations = [];
...
function estimatePage(indexLike) {
    if (typeof indexLike !== "number" || Number.isNaN(indexLike))
        return undefined;
    return Math.max(1, Math.floor(indexLike / 8) + 1);
}
export function buildCaptionAnchorObjectsFromFixture(objects) {
...
export function buildCaptionAnchorObjectsFromParsedContext(ctx) {
    const paragraphMap = new Map((ctx.paragraphs || []).map((paragraph) => [Number(paragraph?.index ?? -1), paragraph]));
    const captionObjects = (ctx.structureItems || [])
        .filter((item) => item?.type === "figure_caption" || item?.type === "table_caption")
        .map((item) => {
    ...
    const figureObjects = (ctx.images || []).map((image, index) => {
    ...
    const tableObjects = (ctx.tables || []).map((table, index) => ({

...
export function makeDownloadingEvent(fixType?: FixType): FixJobEvent {
  return makeFixEvent({
    type: "stage",
    stage: "downloading",
    title: "原稿已读取",
...
export function makeArtifactEvent(fixType: FixType, artifact: FixJobArtifact): FixJobEvent {
  return makeFixEvent({
    type: "artifact",
    stage: `fixed:${fixType}`,
...
export function makeUploadingEvent(): FixJobEvent {
  return makeFixEvent({
    type: "stage",
    stage: "uploading",
    title: "正在保存修复稿",
...
export function makeDoneEvent(isPassthrough: boolean): FixJobEvent {
  return makeFixEvent({
    type: "stage",
    stage: "done",
...
export function makeFailedEvent(errorMessage: string): FixJobEvent {
  return makeFixEvent({
    type: "error",
    stage: "error",
    title: "修复任务失败",

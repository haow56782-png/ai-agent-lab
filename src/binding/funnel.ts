...
export interface StageFunnel {
  ...
  progressed: number;
  conversionRate: number;   // progressed / entered
...
function stageOrder(stage: BindingState): number {
  const order: BindingState[] = [
    "INIT",
    "URL_INPUT",
    "INVALID_URL",
    "SITE_RECOGNIZING",
...
const FAILURE_STATES: Set<BindingState> = new Set([
  "INVALID_URL",
  "UNSUPPORTED_SITE",
  "AUTH_REJECTED",
  "AUTH_FAILED",
...
export function analyzeFunnel(events: RuntimeEvent[][]): FunnelReport {
  const totalSessions = events.length;
  let successfulSessions = 0;

  // Count: for each main flow stage, how many sessions entered/completed it
  const entered = new Map<BindingState, number>();
  const completed = new Map<BindingState, number>();
  const failed = new Map<BindingState, number>();

  for (const sessionEvents of events) {
    let reachedSignalReady = false;
    const seen = new Set<BindingState>();
    ...
    if (reachedSignalReady) successfulSessions++;
  }
...
export function formatFunnelReport(report: FunnelReport): string {
  ...
  for (const s of report.stages) {
    lines.push(
      `  ${s.stage.padEnd(28)} ${String(s.entered).padEnd(7)} ` +

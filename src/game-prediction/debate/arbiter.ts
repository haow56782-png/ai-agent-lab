...
import type { AgentDebateOutput, AgentTraceEntry, ConsensusLevel, FinalDebateOutput, Stance } from "./types.js";

/* ─── Helpers ─── */

function makeTrace(agents: AgentDebateOutput[]): AgentTraceEntry[] {
 ...
 */
function downgradeAction(action: string): "PLAY" | "SKIP" | "REDUCE_SIZE" | "STOP_SESSION" {
  switch (action) {
 ...
 */
function weightedVote(agents: AgentDebateOutput[]): number {
  let total = 0;
 ...
 */
function determineConsensus(agents: AgentDebateOutput[]): ConsensusLevel {
  ...
  if (support === agents.length) return "UNANIMOUS";
  if (oppose === 0 && caution <= Math.ceil(agents.length / 3)) return "MAJORITY";
  if (oppose > 0 && support > 0) return "SPLIT";
  if (oppose >= Math.ceil(agents.length / 2)) return "SPLIT";
  return "MAJORITY";
}
 ...
 */
function collectWarnings(agents: AgentDebateOutput[], existingWarnings: string[]): string[] {
  const warnings = new Set<string>();
  for (const existing of existingWarnings) warnings.add(existing);
 ...
 */
function collectAdjustments(agents: AgentDebateOutput[]): string[] {
...
export function finalArbiter(
  ...
  let finalAction: "PLAY" | "SKIP" | "REDUCE_SIZE" | "STOP_SESSION" = decision.action;
  let consensusLevel: ConsensusLevel = determineConsensus(agents);
  ...
  if (hardVeto) {
    ...
    if (vote < -0.3 && finalAction === "PLAY") {
      finalAction = downgradeAction(finalAction);
      if (consensusLevel !== "ARBITER_OVERRIDE") consensusLevel = "ARBITER_OVERRIDE";
  ...
  if (opposeCount >= 2) {
    riskAdjustment = riskLevels[Math.min(currentRiskIndex + 1, riskLevels.length - 1)] as any;
  }
  ...
  return {
    ...
    riskAdjustment: riskAdjustment as any,

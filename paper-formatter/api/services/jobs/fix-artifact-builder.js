...
function pickSourceLine(sourceLines, sourceIndex, fallback) {
    if (sourceLines.length === 0)
        return fallback;
    return sourceLines[sourceIndex % sourceLines.length];
}
export function getFixArtifactDetailText(fixType) {
    return FIX_ARTIFACT_DETAILS[fixType].join("；");
}
export function makeFixArtifact(fixType, sourceContext, sourceIndex, formatterDiff) {

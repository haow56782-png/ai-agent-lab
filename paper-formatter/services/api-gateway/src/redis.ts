...
if (redisTarget) {
  console.log("[redis] Parsed target:", {
    host: redisTarget.host,
    port: redisTarget.port,
    hasPassword: !!redisTarget.password,
  });
} else if (REDIS_URL) {
...
async function send(parts: Array<string | number>): Promise<string | null> {
  ...
  return await new Promise((resolve, reject) => {
    ...
    socket.on("connect", () => {
      console.log("[redis] Connected to Redis at", `${redisTarget.host}:${redisTarget.port}`);
    ...
    socket.on("data", (chunk) => {
      ...
      // Wait for all responses (AUTH + actual command) when password is set
      const expectedLines = redisTarget.password ? 4 : 2;
      if (raw.split("\r\n").filter(Boolean).length < expectedLines) return;
      socket.end();
    });
    socket.on("timeout", () => {
      console.error("[redis] Connection timed out to", `${redisTarget.host}:${redisTarget.port}`, "(2000ms)");
      socket.destroy();
      reject(new Error("Redis timeout"));
    });
    socket.on("error", (err) => {
      console.error("[redis] Socket error connecting to", `${redisTarget.host}:${redisTarget.port}`, "\n", err);
      reject(err);
    });
    socket.on("close", () => {
      try {
        ...
        if (redisTarget.password) {
          ...
          // AUTH response is first 2 lines (+OK\r\n), skip them
          response = lines.slice(2).join("\r\n");
        }
        resolve(parseSimpleResponse(response));
      } catch (err) {
        reject(err);
      }
    });

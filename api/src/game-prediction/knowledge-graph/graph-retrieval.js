 ...
 */
export async function retrieveGraphContext(store, playerId, sessionId) {
    ...
    const repeatedBehaviorEvents = extractBehaviorEvents(riskPatterns, playerId);
    ...
    const recommendedConstraints = generateGraphConstraints(riskPatterns, strategyReliability);
    // Collect warnings
...
/* ─── Helpers ─── */
function extractBehaviorEvents(riskPatterns, _playerId) {
...
async function buildDecisionPathSummary(store, playerId) {
...
function generateGraphConstraints(riskPatterns, strategyReliability) {
    const constraints = [];
    if (riskPatterns.type === "PLAYER_RISK_PATTERNS") {
...
function collectWarnings(riskPatterns, strategyReliability, driftSignals) {
    const warnings = [];
    if (riskPatterns.type === "PLAYER_RISK_PATTERNS") {

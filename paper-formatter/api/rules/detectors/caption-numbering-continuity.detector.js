...
function buildCaptionIndex(structureItems) {
    const figures = [];
    const tables = [];
 ...
 */
function checkContinuity(entries, captionKind) {
    if (entries.length < 2)
        return [];
    const detections = [];
 ...
 */
export function detectCaptionNumberingContinuity(ctx) {
...
export const captionNumberingContinuityDetector = {
    ruleId: CAPTION_NUMBERING_CONTINUITY_RULE_ID,

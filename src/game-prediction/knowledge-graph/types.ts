...
export interface StopSessionPathResult {
  path: { from: string; to: string; via: string; timestamp: string }[];
  rootCauses: string[];
...
export interface GameBehaviorRiskResult {
  gameType: string;
  mostCommonBehaviors: { behavior: string; count: number }[];
  riskLevel: string;
...
export interface LearningDriftResult {
  ...
  downgradeEvents: { action: string; timestamp: string }[];
}
...
export type GraphQueryResult =
  | { type: "PLAYER_RISK_PATTERNS"; data: RiskPatternResult[] }
  | { type: "STRATEGY_RELIABILITY"; data: StrategyReliabilityResult[] }
  | { type: "STOP_SESSION_PATH"; data: StopSessionPathResult }
  | { type: "GAME_BEHAVIOR_RISKS"; data: GameBehaviorRiskResult }
  | { type: "LEARNING_DRIFT_DOWNGRADE"; data: LearningDriftResult[] }
  | { type: "DEBATE_MOST_OPPOSED"; data: DebateOppositionResult[] };
...
/* ─── Graph Context (for engine injection) ─── */

export interface GraphContext {
  playerId: string;
  relatedRiskPatterns: RiskPatternResult[];
  strategyReliability: StrategyReliabilityResult[];
  repeatedBehaviorEvents: { type: string; count: number; lastSeen: string }[];
